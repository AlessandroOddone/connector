...
class ContentValidationTest {
  ...
  @Test fun `Unknown return type generic argument`() {
    val sourceFile = SourceFile.kotlin(
      ...
      @Service interface TestApi {
        @GET("get") suspend fun get(): Map<TypeThatDoesNotExist, OtherTypeThatDoesNotExist>
      }
      """
    )
  ...
  @Test fun `@Part must provide formFieldName for @Multipart(form-data) when the parameter type is NOT PartData`() {
    val sourceFile = SourceFile.kotlin(
      "Test.kt",
      """
      package test
    ...
    sourceFile.runTestCompilation {
      assertKspErrors(
        "When the @Multipart subtype is 'form-data' (the default), @Part must provide a 'formFieldName' or use PartData as the parameter type." atLine 11,
        "When the @Multipart subtype is 'form-data' (the default), @Part must provide a 'formFieldName' or use PartData as the parameter type." atLine 15,
        "When the @Multipart subtype is 'form-data' (the default), @Part must provide a 'formFieldName' or use PartData as the parameter type." atLine 19,
      )
    }
  }

  @Test fun `@PartIterable must provide formFieldName for @Multipart(form-data) when the iterable type argument is NOT PartData`() {
    val sourceFile = SourceFile.kotlin(
      "Test.kt",
      """
      package test
    ...
    sourceFile.runTestCompilation {
      assertKspErrors(
        "When the @Multipart subtype is 'form-data' (the default), @PartIterable must provide a 'formFieldName' or use parts of type PartData." atLine 11,
        "When the @Multipart subtype is 'form-data' (the default), @PartIterable must provide a 'formFieldName' or use parts of type PartData." atLine 15,
        "When the @Multipart subtype is 'form-data' (the default), @PartIterable must provide a 'formFieldName' or use parts of type PartData." atLine 19,
      )
    }
  }
